...
module Coq_sforces_cbn = 
 ...
 struct 
  ...
  let sforces_mon a w annot h w' h0 =
    match a with
    | Func (f, f0) ->
    ...
    | Sum (f, f0) ->
      (match Obj.magic h with
       | Inl h' ->
         Obj.magic (Inl (Coq_ks_monad.coq_Kont_mon w annot f h' w' h0))
       | Inr h' ->
         Obj.magic (Inr (Coq_ks_monad.coq_Kont_mon w annot f0 h' w' h0)))
    | x -> Obj.magic (Coq_ks.coq_X_mon w annot x (Obj.magic h) w' h0)
  
  (** val coq_Kont_sforces_mon2' :
      Coq_ks.coq_K -> bool -> formula -> (Coq_ks.coq_K -> sforces -> sforces)
      -> sforces Coq_ks_monad.coq_Kont -> bool -> sforces
      Coq_ks_monad.coq_Kont **)
  
  let coq_Kont_sforces_mon2' w annot a sforces_mon3 h annot' =
    match annot with
    | True ->
      (match annot' with
       | True -> h
  ...
  let rec sforces_mon2 f w h =
    ...
    | Sum (f0, f1) ->
      (match Obj.magic h with
       | Inl k ->
         Obj.magic (Inl
           (coq_Kont_sforces_mon2' w False f0 (sforces_mon2 f0) k True))
       | Inr k ->
         Obj.magic (Inr
           (coq_Kont_sforces_mon2' w False f1 (sforces_mon2 f1) k True)))
  ...
  let coq_Kont_sforces_mon2 w annot a x annot' =
    coq_Kont_sforces_mon2' w annot a (sforces_mon2 a) x annot'
...
module Soundness = 
 ...
 struct 
  ...
  module Coq_cbn_validity = Coq_sforces_cbn(Coq_ks)
  
  module Coq_generic_properties = Coq_forcing_generic_properties(Coq_ks)(Coq_cbn_validity)
  
  (** val soundness :
      context -> bool -> formula -> proof -> Coq_ks.coq_K -> bool ->
      Coq_generic_properties.coq_Kont_cxt -> Coq_cbn_validity.sforces
      Coq_cbn_validity.Coq_ks_monad.coq_Kont **)
  
  let rec soundness c b f p w annot' h' =
    match p with
    | Inl0 (gamma, annot, a, b0, p0) ->
      Coq_generic_properties.ret w annot' (Sum (a, b0))
        (Obj.magic (Inl (soundness gamma annot a p0 w annot' h')))
    | Inr0 (gamma, annot, a, b0, p0) ->
      Coq_generic_properties.ret w annot' (Sum (a, b0))
        (Obj.magic (Inr (soundness gamma annot b0 p0 w annot' h')))
      ...
      Coq_generic_properties.ret w annot' (Func (a, b0))
        (Obj.magic (fun w1 wle1 annot1 _ h1 ->
          Obj.magic (fun w0 annot'0 _ h'0 ->
            ...
            __ (Pair (h1,
            (Coq_generic_properties.coq_Kont_cxt_mon gamma w annot1
              (Coq_generic_properties.coq_Kont_sforces_cxt_mon2 gamma w
                annot' h' annot1) w1 wle1)))))
    | Hyp (gamma, annot, a) -> let Pair (ha, hGamma) = Obj.magic h' in ha
      ...
      Coq_generic_properties.bind w annot' (Sum (a, b0)) c0
        ...
        | Inl k ->
          Obj.magic (fun w0 annot'0 _ h'0 ->
            ...
            __ (Pair (k,
            (Coq_generic_properties.coq_Kont_cxt_mon gamma w annot' h' w1
              wle1)))
        | Inr k ->
          Obj.magic (fun w0 annot'0 _ h'0 ->
    ...
    | App (gamma, annot, a, b0, p0, p1) ->
      Coq_generic_properties.bind w annot' (Func (a, b0)) b0
        (fun w1 wle1 h1 ->
        Obj.magic h1 w1 (Coq_ks.wle_refl w1) annot' __
               ...
               (Coq_cbn_validity.run w True
                 (soundness gamma True Bot p0 w True
                   (Coq_generic_properties.coq_Kont_sforces_cxt_mon2 gamma w
                     False h' True)))))
    | Shift (gamma, a, p0) ->
      let x0 = fun w1 leb1 k1 ->
        Coq_cbn_validity.run w1 True
          (Obj.magic (fun w0 annot'0 _ h'0 ->
...
module Completeness = 
 struct 
  ...
  module Coq_cbn_validity = Coq_sforces_cbn(U)
  
  module Coq_generic_properties = Coq_forcing_generic_properties(U)(Coq_cbn_validity)
  
  (** val completeness :
      formula -> context -> bool -> (Coq_cbn_validity.sforces
      Coq_cbn_validity.Coq_ks_monad.coq_Kont -> proof_nf, proof_ne ->
      Coq_cbn_validity.sforces Coq_cbn_validity.Coq_ks_monad.coq_Kont) prod **)
         ...
         Obj.magic h1
           ...
           Obj.magic h w2
             ...
             f3 w3 (U.wle_refl w3) True __
               (Coq_generic_properties.ret w3 True f0
                 (Coq_cbn_validity.sforces_mon f0 w2 True h2 w3 incl3)) w3
           ...
           x0
             ...
             Obj.magic h w1 w2
               ...
               f3 w3 (U.wle_refl w3) False __
                 (Coq_generic_properties.ret w3 False f0
                   (Coq_cbn_validity.sforces_mon f0 w2 False h2 w3 incl3)) w1
                 w3 (U.wle_refl w3) (fun w4 incl4 h4 ->
                 k2 w4
      ...
      (Obj.magic (fun w' incl' annot' _ h' ->
        ...
        x0 (Ne_App (w', annot', f0, f1,
          (proof_ne_mon w w' incl' (Func (f0, f1)) annot'
            (proof_ne_mon2 (Func (f0, f1)) w annot e annot')), h'0)))))))
  | Sum (f0, f1) ->
    let iHA1 = completeness f0 in
    let iHA2 = completeness f1 in
    (fun w annot -> Pair ((fun h ->
        ...
        (Obj.magic h (Cons ((Func ((Sum (f0, f1)), Bot)), w)) (Prefix_cons
          ...
          | Inl h11 ->
            Ne_App (w1, True, (Sum (f0, f1)), Bot,
              ...
              (let annot0 = True in
               let Pair (x, x0) = iHA1 w1 annot0 in x h11))))
          | Inr h12 ->
            Ne_App (w1, True, (Sum (f0, f1)), Bot,
              (proof_ne_mon (Cons ((Func ((Sum (f0, f1)), Bot)), w)) w1 incl1
                (Func ((Sum (f0, f1)), Bot)) True (Ne_Hyp (w, True, (Func
              ...
              (let annot0 = True in
               let Pair (x, x0) = iHA2 w1 annot0 in x h12)))))))))
    | False ->
      Obj.magic h (Sum (f0, f1)) w (U.wle_refl w) (fun w1 incl1 h1 ->
        match h1 with
        | Inl h11 ->
          Nf_Inl (w1, False, f0, f1,
            (let annot0 = False in let Pair (x, x0) = iHA1 w1 annot0 in x h11))
        | Inr h12 ->
          Nf_Inr (w1, False, f0, f1,
            (let annot0 = False in let Pair (x, x0) = iHA2 w1 annot0 in x h12)))),
    (fun e ->
    match annot with
    | True ->
         ...
         Obj.magic h (Ne_Hyp (w1, True, f0)) (Cons (f0, w1))
           ...
           k1 w2
             (U.wle_trans w1 (Cons (f0, w1)) w2 (Prefix_cons (w1, w1, f0,
               (Prefix_refl w1))) incl2) (Inl
             (Coq_generic_properties.ret w2 True f0 h2)))))), (Nf_ne ((Cons
        (f1, w1)), True, Bot,
         ...
         Obj.magic h (Ne_Hyp (w1, True, f1)) (Cons (f1, w1))
           ...
           k1 w2
             (U.wle_trans w1 (Cons (f1, w1)) w2 (Prefix_cons (w1, w1, f1,
               (Prefix_refl w1))) incl2) (Inr
             (Coq_generic_properties.ret w2 True f1 h2))))))))
    | False ->
            ...
            (let hhypo = Ne_Hyp (w1, False, f0) in
             snd (Obj.magic iHA1 (Cons (f0, w1)) False) hhypo (Atom a) (Cons
               ...
               k1 w2
                 (U.wle_trans w1 (Cons (f0, w1)) w2 (Prefix_cons (w1, w1, f0,
                   (Prefix_refl w1))) incl2) (Inl
                 (Coq_generic_properties.ret w2 False f0 h2)))))), (Nf_ne
            ((Cons (f1, w1)), False, (Atom a),
            (let hhypo = Ne_Hyp (w1, False, f1) in
             snd (Obj.magic iHA2 (Cons (f1, w1)) False) hhypo (Atom a) (Cons
               ...
               k1 w2
                 (U.wle_trans w1 (Cons (f1, w1)) w2 (Prefix_cons (w1, w1, f1,
                   (Prefix_refl w1))) incl2) (Inr
                 (Coq_generic_properties.ret w2 False f1 h2)))))))
        | Bot ->
            ...
            (let hhypo = Ne_Hyp (w1, False, f0) in
             snd (Obj.magic iHA1 (Cons (f0, w1)) False) hhypo (Func (c1, c2))
               ...
               k1 w2
                 (U.wle_trans w1 (Cons (f0, w1)) w2 (Prefix_cons (w1, w1, f0,
                   (Prefix_refl w1))) incl2) (Inl
                 (Coq_generic_properties.ret w2 False f0 h2)))),
            (let hhypo = Ne_Hyp (w1, False, f1) in
